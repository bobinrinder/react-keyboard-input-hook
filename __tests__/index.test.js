...
;
import useKey, { useKeyUp, useKeyDown, useKeyCombo, useFireTvKeyUp, useFireTvKeyDown, checkIfArrayItemsinArray } from '../index';
...
describe('useKey setup', () => {
    ...
    test('if key event is invalid it passes with warnings', () => {
        ...
        expect(console.warn).toHaveBeenCalledWith("useKey keyEvent invalid, assumed keyEvent 'keydown' as fallback!");
    });
    ...
    test('if key codes array for combo is too short it passes with warnings', () => {
        ...
        expect(console.warn).toHaveBeenCalledWith("Invalid arguments for usekeyCombo!");
    });
...
describe('events', () => {
    ...
    test('callback is called with the right values on keyup event with whitelist items', async () => {
        const callback = jest.fn();
        const { container } = render(<UseKeyUpTestComponent callback={callback} whitelist={[39,40]} />);
        const keyUpEvent = new KeyboardEvent('keyup', {
            key: 'ArrowRight',
            bubbles: true,
            keyCode: 39,
        ...
        expect(callback).toHaveBeenCalledWith({ "e": keyUpEvent, "keyCode": 39, "keyName": "ArrowRight" });
        expect(callback).toHaveBeenCalledWith({ "e": keyUpEvent, "keyCode": 40, "keyName": "ArrowDown" });
        expect(callback).toHaveBeenCalledTimes(2);
    });

    test('callback is called with the right values on keyup event with blacklist items', async () => {
        const callback = jest.fn();
        const { container } = render(<UseKeyUpTestComponent callback={callback} blacklist={[39, 40]} />);
        const keyUpEvent = new KeyboardEvent('keyup', {
            key: 'ArrowRight',
            bubbles: true,
            keyCode: 39,
        ...
        fireEvent(container, keyUpEvent2);
        fireEvent(container, keyUpEvent3);
        expect(callback).toHaveBeenCalledWith({ "e": keyUpEvent, "keyCode": 37, "keyName": "ArrowLeft" });
        expect(callback).toHaveBeenCalledTimes(1);
    });
